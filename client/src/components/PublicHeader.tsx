...
export default function PublicHeader() {
  ...
  const copy = isBn ? { features: "ফিচার", workflow: "কীভাবে কাজ করে", blog: "ব্লগ", generator: "Invoice তৈরি করুন", signIn: "সাইন ইন", start: "ফ্রি শুরু করুন", menu: "মেনু খুলুন", home: "invoice-gen.net home" } : { features: "Features", workflow: "How it works", blog: "Blog", generator: "Create invoice", signIn: "Sign in", start: "Get started free", menu: "Open menu", home: "invoice-gen.net home" };
  ...
  return <header className="relative z-30 border-b border-slate-200/80 bg-white/95 backdrop-blur">
    ...
    {open ? <nav className="border-t border-slate-100 bg-white px-5 py-4 md:hidden" aria-label="Mobile navigation"><div className="grid gap-1.5"><a href="/#features" onClick={close} className="rounded-lg px-3 py-2.5 text-sm font-semibold text-slate-700 hover:bg-blue-50 hover:text-blue-700">{copy.features}</a><a href="/#workflow" onClick={close} className="rounded-lg px-3 py-2.5 text-sm font-semibold text-slate-700 hover:bg-blue-50 hover:text-blue-700">{copy.workflow}</a><Link href="/blog" onClick={close} className="rounded-lg px-3 py-2.5 text-sm font-semibold text-slate-700 hover:bg-blue-50 hover:text-blue-700">{copy.blog}</Link><Link href="/invoice-generator" onClick={close} className="rounded-lg px-3 py-2.5 text-sm font-semibold text-blue-700 hover:bg-blue-50">{copy.generator}</Link><div className="mt-2 flex gap-2"><button onClick={toggleLocale} className="inline-flex flex-1 items-center justify-center gap-2 rounded-lg border border-slate-200 px-3 py-2.5 text-sm font-semibold text-slate-700"><Globe2 className="h-4 w-4" />{isBn ? "English" : "বাংলা"}</button><Link href="/invoice-generator" onClick={close} className="flex-1 rounded-lg bg-blue-700 px-3 py-2.5 text-center text-sm font-bold text-white">{copy.start}</Link></div></div></nav> : null}
  </header>;
}
